...
struct DocAccessData
{
	...
	// sets timeout value for LoadData(), default 100 ms
	void SetLoadTimeoutValue(long micro_seconds);
...
private:
	// checks if device's address is localhost, 
	// and if so then sets timeout for LoadData()
	void SetLoadTimeout();

	timeval			_timeout;	// timeout for function select in LoadData(), default 100 ms
	const timeval*	_ptimeout;	// pointer to _timeout
};
...
private:
	...
	SrvEventCallback& operator= (const SrvEventCallback& srcobj) {return *this;}
};
...
private:
	...
	DevFinderCallback& operator= (const DevFinderCallback& srcobj) {return *this;}
};
